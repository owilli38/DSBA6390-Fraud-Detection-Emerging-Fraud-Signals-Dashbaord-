...
st.markdown(
    ...
    /* Remove all default top padding from the sidebar inner div */
    ...
    .nav-btn.active {
        ...
        color: white !important;
        font-weight: 700;
        border-left: 3px solid #20B07A;
    }

    /* Override Streamlit button padding inside sidebar */
    ...
    /* Active nav button — primary type gives it colour */
    section[data-testid="stSidebar"] .stButton > button[kind="primary"] {
        background: rgba(255,255,255,0.14) !important;
        color: white !important;
        font-weight: 700 !important;
        border-left: 3px solid #20B07A !important;
    }
    ...
    .page-header {
        ...
        color: white;
        margin-bottom: 1rem;
        box-shadow: 0 6px 18px rgba(13, 43, 82, 0.12);
        display: flex;
        align-items: center;
    ...
    .alert-high   { border-left: 5px solid #D92D20; }
    .alert-medium { border-left: 5px solid #F79009; }
    .alert-low    { border-left: 5px solid #12B76A; }
    </style>
    """,
...
def fetch_executive_stats() -> dict:
    ...
    try:
        ...
        return {
            ...
            "cluster_delta": f"+ {emerging_count} emerging",
...
def render_sidebar():
    """
    Render the left navigation sidebar with logo flush to top and full-width nav buttons.

    Replaces the radio widget with st.button calls keyed to session state so each
    page button fills the sidebar width and the active page is highlighted with a
    green left border via the primary button type.  The logo renders with zero top
    margin so there is no dead space at the top of the sidebar.
    ...
    with st.sidebar:
        # Logo flush to top — negative margin collapses Streamlit's default padding
        st.markdown(
            '<div style="margin-top:-3rem;padding-top:0;">',
            unsafe_allow_html=True,
        )
        if LOGO_PATH.exists():
            ...
            st.markdown(
                ...
                'padding:0.5rem 0;">Pickaxe Analytics</div>',
                unsafe_allow_html=True,
            )
        ...
        st.markdown(
            ...
            'text-transform:uppercase;letter-spacing:0.08em;padding-left:0.25rem;">'
            'Navigation</div>',
            unsafe_allow_html=True,
        )
...
def render_page_header(title: str, description: str):
    """
    Render the full-width page header with title on the left and subtitle on the right.

    Uses a flex row layout — title anchored left, subtitle anchored right — so the
    banner spans the full content width with no wasted vertical space.

    Args:
        title:       Main page heading (left-aligned, large bold).
        description: One-line subtitle (right-aligned, smaller, capped at 55% width).
    """
    st.markdown(
        f'<div class="page-header">'
        f'<div class="page-title">{_esc(title)}</div>'
        f'<div class="page-subtitle">{_esc(description)}</div>'
        f'</div>',
        unsafe_allow_html=True,
    )
